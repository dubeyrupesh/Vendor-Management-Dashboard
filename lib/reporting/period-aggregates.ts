...
export async function getVendorPeriodRows(selection: PeriodSelection): Promise<{
  ...
  const rows: VendorPeriodRow[] = vendors
    .map((vendor) => {
      ...
      const scoreBreakdownProjects: BreakdownProject[] = [...breakdownByProject.entries()].map(
        ([projectId, list]) => {
          ...
          return {
            ...
            automationCoverageScore: averageNumbers(list.map((item) => item.automationCoverageScore)),
            xrayAutomationRatioScore: averageNumbers(list.map((item) => item.xrayAutomationRatioScore)),
            qualitativeScore: averageNumbers(list.map((item) => item.qualitativeScore)),
            prodDefectLeakageScore: averageNumbers(list.map((item) => item.prodDefectLeakageScore)),
            meetsCoverageTarget:
              averageNumbers(list.map((item) => item.automationCoverageScore)) > AUTOMATION_COVERAGE_TARGET,
          };
        },
      );
      ...
      const projects = vendor.projects.map((project) => {
        ...
        return {
          ...
          responsiveness: responsiveness.length ? clampQualitative(averageNumbers(responsiveness)) : 0,
          availability: availability.length ? clampQualitative(averageNumbers(availability)) : 0,
          complexityUnderstanding: complexity.length ? clampQualitative(averageNumbers(complexity)) : 0,
